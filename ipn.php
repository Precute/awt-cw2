...
// Invalid IPN transaction.  Check the $ipn_status and log for details.
	...
	mail("harpalikli@hotmail.com","My subject",print_r($request,true));
	$im_debut_ipn=true;
...
$payment_date_raw=$this->issetCheck($post,'payment_date');
$num_cart_items = $this->issetCheck($post,'num_cart_items');

date_default_timezone_set("Europe/London");
$payment_date_split = strtotime($payment_date_raw);
$payment_date = date('Y-m-d H:i:s', $payment_date_split);

mysqli_query($con,"INSERT INTO transactions (payer_email,first_name,last_name,amount,currency,country,txn_id,txn_type,payment_status,payment_type,payment_date, num_cart_items) 
VALUES ('$payer_email','$first_name','$last_name','$amount','$currency','$country','$txn_id','$txn_type','$payment_status','$payment_type', '$payment_date','$num_cart_items')");
$tid = mysqli_insert_id($con);
$orderid = $this->issetCheck($post,'custom');
mysqli_query($con, "UPDATE orders SET transactionid = $tid WHERE id=$orderid ");
error_log("Error: %s.\n" . mysqli_error($con));
mysqli_close($con);
